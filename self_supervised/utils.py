...
@attr.s(auto_attribs=True, slots=True)
class Inat2021Dataset(DatasetBase):
    transform_train: Callable[[Any], torch.Tensor] = imagenet_default_transform
    transform_test: Callable[[Any], torch.Tensor] = imagenet_default_transform

    def configure_train(self):
        assert os.path.exists(self.data_path + "/semi_inat/l_train")
        return ImageFolder(self.data_path + "/semi_inat/l_train", transform=self.transform_train)

    def configure_validation(self):
        assert os.path.exists(self.data_path + "/semi_inat/val")
        return ImageFolder(self.data_path + "/semi_inat/val", transform=self.transform_test)
...
def get_moco_dataset(name: str, t: MoCoTransforms) -> DatasetBase:
    if name == "stl10":
        ...
        return Inat2021Dataset(transform_train=t.split_transform, transform_test=t.get_test_transform())

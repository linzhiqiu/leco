...
class DistillSoft(SSLObjective):
    ...
    def __init__(self, model_T, T=1.0, hierarchical_supervision=None, edge_matrix=None):
    ...
    def masked_kl(self, model, outputs_s, inputs, labels):
        with torch.no_grad():
            outputs_t = self.calc_output(model_T, inputs)
            p_t = self.condition_outputs_for_probs(outputs_t/self.T, labels)
        ...
        kld_loss = kld_loss * (self.T**2) * mask
...
class DistillHard(SSLObjective):
    ...
    def __init__(self, model_T, hierarchical_supervision=None, edge_matrix=None):
